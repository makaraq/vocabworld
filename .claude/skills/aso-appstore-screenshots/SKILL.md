...
**CRITICAL — Use exactly ONE Bash tool call for all 3 crop/resize operations.** Do NOT make 3 separate Bash calls. Do NOT use parallel Bash calls. Use the single command below (cross-platform — uses `crop_resize.py`, not the macOS-only `sips`) so the user only sees one permission prompt:

```bash
SKILL_DIR="$HOME/.claude/skills/aso-appstore-screenshots" && \
(command -v python3 >/dev/null 2>&1 && PY=python3 || PY=python) && \
"$PY" "$SKILL_DIR/crop_resize.py" \
  screenshots/01-[benefit-slug]/v1.jpg \
  screenshots/01-[benefit-slug]/v2.jpg \
  screenshots/01-[benefit-slug]/v3.jpg \
