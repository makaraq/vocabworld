/**
 * Translate Verbs using Llama 3.1
 * Clean, strict validation with proper punctuation handling
 * 
 * Usage: node scripts/translate-verbs-llama.js
 */
...
const MODEL = 'llama3.2:3b';  // Smaller model that fits in RAM
...
// ═══════════════════════════════════════════════════════════════════
// SCRIPT VALIDATORS - Pure script detection
// ═══════════════════════════════════════════════════════════════════

const SCRIPTS = {
  arabic: {
    name: 'Arabic',
    test: (t) => /^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\s\u0640]+$/.test(t) && /[\u0600-\u06FF]/.test(t)
  },
  cyrillic: {
    name: 'Cyrillic', 
    test: (t) => /^[\u0400-\u04FF\s\-]+$/.test(t) && /[\u0400-\u04FF]/.test(t)
  },
  bengali: {
    name: 'Bengali',
    test: (t) => /^[\u0980-\u09FF\s]+$/.test(t) && /[\u0980-\u09FF]/.test(t)
  },
  greek: {
    name: 'Greek',
    test: (t) => /^[\u0370-\u03FF\u1F00-\u1FFF\s\-]+$/.test(t) && /[\u0370-\u03FF]/.test(t)
  },
  hebrew: {
    name: 'Hebrew',
    test: (t) => /^[\u0590-\u05FF\uFB1D-\uFB4F\s]+$/.test(t) && /[\u0590-\u05FF]/.test(t)
  },
  devanagari: {
    name: 'Devanagari',
    test: (t) => /^[\u0900-\u097F\s]+$/.test(t) && /[\u0900-\u097F]/.test(t)
  },
  japanese: {
    name: 'Japanese',
    test: (t) => /^[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF\s]+$/.test(t) && /[\u3040-\u30FF\u4E00-\u9FFF]/.test(t)
  },
  georgian: {
    name: 'Georgian',
    test: (t) => /^[\u10A0-\u10FF\u2D00-\u2D2F\s]+$/.test(t) && /[\u10A0-\u10FF]/.test(t)
  },
  korean: {
    name: 'Korean',
    test: (t) => /^[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\s]+$/.test(t) && /[\uAC00-\uD7AF]/.test(t)
  },
  thai: {
    name: 'Thai',
  ...
  chinese: {
    name: 'Chinese',
    test: (t) => /^[\u4E00-\u9FFF\u3400-\u4DBF\s]+$/.test(t) && /[\u4E00-\u9FFF]/.test(t)
  },
  latin: {
    name: 'Latin',
...
// ═══════════════════════════════════════════════════════════════════
// TEXT CLEANING - Strip everything except the word
// ═══════════════════════════════════════════════════════════════════
...
function cleanText(raw) {
  ...
  t = t.replace(/^(Translation|Answer|Result|Output|The\s+\w+\s+(word|translation|verb)\s*(is|for)?|In\s+\w+)[:\s]*/i, '');
  ...
  // Remove all punctuation at start/end
  t = t.replace(/^[.,!?;:*#@\-–—]+|[.,!?;:*#@\-–—]+$/g, '');
  
  // Remove punctuation with spaces around it
  t = t.replace(/\s*[.,!?;:*#@]+\s*/g, ' ');
  
  // Take first part if separated by dash/colon with explanation
  t = t.split(/\s+[-–—:]\s+/)[0];
  
  // Take first if / separated
...
function validate(text, langCode, langInfo, original) {
  if (!text || text.length < 1) return { ok: false, why: 'empty' };
  if (text.length > 30) return { ok: false, why: 'too_long' };
  
  const words = text.split(/\s+/).filter(w => w);
  if (words.length > 4) return { ok: false, why: 'too_many_words' };
  
  // Check script
  const scriptValidator = SCRIPTS[langInfo.script];
  if (!scriptValidator) return { ok: false, why: 'unknown_script' };
  
  if (!scriptValidator.test(text)) {
    return { ok: false, why: 'wrong_script' };
  }
  
  // For latin scripts, check it's not just English
  if (langInfo.script === 'latin') {
    const lower = text.toLowerCase();
    if (lower === original.toLowerCase()) {
      // Allow cognates only for Romance languages
      const cognateOK = ['es', 'fr', 'it', 'pt', 'ca', 'ro'].includes(langCode);
      if (!cognateOK) return { ok: false, why: 'same_as_english' };
    }
  }
...
async function translate(verb, langCode, langInfo) {
  const prompts = [
    `Translate the verb "${verb}" to ${langInfo.name}. Reply with ONLY the ${langInfo.name} word:`,
    `${langInfo.name} word for "${verb}":`,
    `"${verb}" in ${langInfo.name} is:`,
    `Give me just the ${langInfo.name} translation of "${verb}":`
  ];
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    ...
    const result = validate(cleaned, langCode, langInfo, verb);
...
async function checkOllama() {
  try {
    ...
    const data = await res.json();
    const has = data.models?.some(m => m.name.includes('llama3.2'));
    if (!has) {
...
async function main() {
  console.log('\n' + '═'.repeat(60));
  console.log('  VERB TRANSLATOR - Llama 3.1');
  ...
  console.log(`  Total: ${verbs.length * langs.length} translations\n`);
  ...
  for (let i = startIdx; i < langs.length; i++) {
    ...
    const pct = Math.round((result.ok / verbs.length) * 100);
    console.log(`   Done: ${result.ok}/${verbs.length} (${pct}%)\n`);
  }
  ...
  if (needsReview.length > 0) {
    ...
    for (const s of needsReview) {
      console.log(`    ${s.name}: ${s.fail} failed`);
    }
    fs.writeFileSync(
