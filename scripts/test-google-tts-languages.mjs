/**
 * Test Google Cloud TTS for 47 languages with 1 sample phrase
 * 
 * This generates audio for "Good morning" in all supported languages
 * to verify TTS quality before bulk generation.
 * 
 * Prerequisites:
 * - npm install @google-cloud/text-to-speech
 * - Set GOOGLE_APPLICATION_CREDENTIALS environment variable
 * 
 * Usage: node scripts/test-google-tts-languages.mjs
 */
...
async function main() {
  console.log('🎵 Testing Google Cloud TTS - 47 Languages');
  ...
  if (!process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    ...
    console.log('Set this to your Google Cloud service account JSON file path.');
    console.log('Example: $env:GOOGLE_APPLICATION_CREDENTIALS = "path\\to\\key.json"');
    process.exit(1);
  }
  ...
  for (const langCode of languages) {
    ...
    if (!ttsCode) {
      ...
      console.log(`   ⏭️  ${langCode}: Not supported by Google TTS`);
      continue;
    }
    ...
    if (audioBuffer) {
      ...
      console.log(`   ❌ ${langCode}: Failed to generate`);
    }
