/**
 * Generate Google TTS Audio - Essential Words (Topic 43)
 * 128 words × 47 languages = 6,016 MP3 files
...
const TOPIC_ID = 43;
const TOPIC_FOLDER = 'EssentialWords';
const OUTPUT_DIR = `scripts/essential-words-audio/${TOPIC_FOLDER}`;
...
async function main() {
  console.log('🎙️  ESSENTIAL WORDS AUDIO GENERATION');
  ...
  console.log(`✅ ${vocabulary.length} words\n`);
  ...
  console.log(`   Target: ${vocabulary.length} words × 47 languages = ${vocabulary.length * 47} files\n`);
  ...
  for (let i = 0; i < vocabulary.length; i++) {
    ...
    console.log(`\n[${i + 1}/${vocabulary.length}] "${englishWord}" (ID: ${vocab.id})`);
    
    // Process all languages including English
    const languagesToProcess = [
      { code: 'en', text: englishWord },
  ...
  const csvFilename = `scripts/essential-words-b2-urls-${timestamp}.csv`;
  ...
  console.log('\n📦 NEXT STEPS:');
  console.log('   1. Upload files to B2: scripts/essential-words-audio/');
  console.log('   2. Update B2 URLs in CSV (replace with full URLs)');
  console.log('   3. Merge CSV with public/data/common-phrases-b2-urls.csv');
  console.log('\n🎉 Generation complete!');
}
